    ...
    <div className='bg-white rounded-xl flex justify-center py-10'>
      <form className='flex flex-col items-end justify-center gap-6' onSubmit={(e) => handleSubmit(e)}>
        ...
        {inputs.map((input, index) => {
          return (
            <FormRow key={index} direction='row' onChange={changeHandler} value={values[input.label]} {...input} />
          );
        })}
        <div>
          <Button className='px-11 mr-2' type='button' onClick={clearValues}>
            Clear
          </Button>
          <Button className=' px-11' background color='white'>
            Add
          </Button>
        </div>
      </form>
